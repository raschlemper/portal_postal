...
app.controller('LancamentoController', 
    ...
    'CentroCustoService', 'ConfiguracaoService', 'ReportService', 'ModalService', 'DateService', 'ListaService', 'PeriodoService', 
    ...
    function ($scope, $filter, LancamentoService, LancamentoTransferenciaService, LancamentoConciliadoService, ContaService, PlanoContaService, 
        CentroCustoService, ConfiguracaoService, ReportService, ModalService, DateService, ListaService, PeriodoService, LancamentoHandler, 
            ...
            $scope.linha = {
                ...
                events: { 
                    edit: function(lancamento) {
                        ...
                        if(validaConciliado(lancamento.idLancamento)) {
                            modalConfirmarConciliado().then(function() {
                                $scope.editar($scope.lancSearch.conta, lancamento, false);
                            });
                        } else {
                            $scope.editar($scope.lancSearch.conta, lancamento, false);
                        }
                    ...
                    remove: function(lancamento) {
                        ...
                        if(validaConciliado(lancamento.idLancamento)) {
                            modalConfirmarConciliado().then(function() {
                                $scope.excluir($scope.lancSearch.conta, lancamento);
                            });
                        } else {
                            $scope.excluir($scope.lancSearch.conta, lancamento);
                        }
                    ...
                    view: function(lancamento) {
                        $scope.visualizar($scope.lancSearch.conta, lancamento);
                    },
                    compensar: function(lancamento) {
                        $scope.compensar($scope.lancSearch.conta, lancamento, false);
                    },
                    anexar: function(lancamento) {
                        $scope.editar($scope.lancSearch.conta, lancamento, true);
                    }
                }
            };
            $scope.events = { 
...
//            lista = filterTipo(lista, search);
        ...
        var contas = function() {
            ContaService.getAll()
                .then(function (data) {
                    $scope.contas = data;
                    $scope.lancSearch.conta = $scope.contas[0];
                    planoContas();
                })
                .catch(function (e) {
                    console.log(e);
...
//                        if(!$scope.lancamentosLista.length) { calculateSaldo($scope.lancamentosLista, criarLancamentosLista); }
        ...
        var changeData = function(conta, dataInicio, dataFim) {
            $scope.lancSearch.periodo = null;
            $scope.lancSearch.dataInicio = DateService.date(dataInicio);
            $scope.lancSearch.dataFim = DateService.date(dataFim);
        ...
        var setDataInicio = function(dataInicio) {
            if(dataInicio) { $scope.lancSearch.dataInicio = DateService.date(dataInicio); }
...
//            angular.element("#dataInicio").val(dataInicio.format('DD/MM/YYYY'));
        ...
        var setDataFim = function(dataFim) {
            if(dataFim) { $scope.lancSearch.dataFim = DateService.date(dataFim); }
        ...
        var criarLancamentosLista = function(lancamentos, saldo) {
            ...
            return _.map(lancamentos, function(lancamento) { 
                ...
                if(lancamento.modelo.id === $scope.modelos[1].id || lancamento.modelo.id === $scope.modelos[3].id) { 
                    lancamento.planoConta = { descricao: lancamento.modelo.descricao };
                    lancamento.centroCusto = { descricao: lancamento.modelo.descricao };
                }
...
//            });
        ...
        var calculateSaldoInicialContas = function(saldoInicial) {
            if($scope.lancSearch.conta) return $scope.lancSearch.conta.valorSaldoAbertura + saldoInicial;
...
//            }
        ...
        $scope.conciliar = function(conta) {
            if(!validaContaSelected(conta)) return;
        ...
        var validaContaSelected = function(conta) {
            if(conta) return true;
            modalMessage(MESSAGES.lancamento.conciliar.info.SEM_CONTA_SELECIONADO);
            return false;
        };
