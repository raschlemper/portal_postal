 ...
 * @author Fernando
 */
public class printPDFLabelToken extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String idC = request.getParameter("codCliente");
        int idCli = Integer.parseInt(idC);
        String nomeBD = Controle.contrLogin.cnpjEmpresa(cod).trim();

        String token = (String) request.getParameter("token");
               
        if(token.equals("F133273B9921711BE3r0lZxd8A@F266irj2Dun")){
        ...
        if (cli == null) {
            ...
            try {
                ...
                String sql_query = "SELECT"
                        ...
                        + " WHERE"
                        + " p.idCliente = " + idCliente + " AND id IN (" + ids + ")"
                        + " ORDER BY id;";
