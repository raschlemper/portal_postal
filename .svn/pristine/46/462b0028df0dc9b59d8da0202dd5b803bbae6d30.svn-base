...
app.factory('CentroCustoService', function($http, PromiseService) {    
    ...
    var getChildrenListCentroCusto = function(list, centroCusto) {
        var listSelected = list || [];
        if(!centroCusto.centros) { listSelected.push(centroCusto); }
        else { 
            centroCusto.centros.map(function(centro) {
                if(centro.grupos) { getChildrenListCentroCusto(listSelected, centro); }
                else { listSelected.push(centro); }
             });
         }     
        return listSelected;
    };
