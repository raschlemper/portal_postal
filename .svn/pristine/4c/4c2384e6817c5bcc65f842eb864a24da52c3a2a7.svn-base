 ...
 * @author RICARDINHO
 */
public class ServPreVendaCaixaPostal extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String idObjetoCaixaPostal = request.getParameter("idObjeto");
        ...
        for (int i = 0; i < qtdPostagem; i++) {
            ...
            if (idDestinatario > 0) {
                ...
                objetoInterno.setIdPrePostagem(idPrePostagem);
                CaixaPostalDao caixaPostalDao = new CaixaPostalDao(nomeBD);
                try{
                    caixaPostalDao.atualizaIdPrePostagem(objetoInterno);
                }catch(Exception ex){
                    falha="Ocorreu um erro ao atualizar o código da pré-postagem";
                }
            } else {
        ...
        if(!falha.isEmpty()){
            response.sendRedirect("Cliente/Servicos/pre_postagem_caixa_postal.jsp?idObjeto="+idObjetoCaixaPostal+"&msg=" + falha);
        }

        response.sendRedirect("Cliente/Servicos/pre_postagem_caixa_postal.jsp?enviado=true&idObjeto="+idObjetoCaixaPostal+"&msg=" + falha);
