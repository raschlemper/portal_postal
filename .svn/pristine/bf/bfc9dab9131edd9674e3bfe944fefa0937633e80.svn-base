...
public class ConversorModeloDestinatarioCSV implements ConversorDestinatarioModelo{

    private final String layoutNaoEncontrado = "Modelo do layout não encontrado";
    private List<LayoutImportacao> layoutImportacao;
    private final int redutorPosicao=-1;
    private final String limitador = ";";

    public ConversorModeloDestinatarioCSV(String nomeModelo, EntityManager manager) {
        layoutImportacao = findModelo(nomeModelo, manager);
    }
    ...
    private int pularQuantidadeDeLinhas() {
        Integer quantidade = findAtribute(MapeamentoLayout.PARAMETROPULALINHA).getPosicao();
        return quantidade==null?0:quantidade;
    }

    private List<LayoutImportacao> findModelo(String nomeModelo, EntityManager manager) {
    ...
    public String getAtributoDaLinhaDoArquivo(String atributo, String linha) {
        ...
        try{
            if (linha.length() > 0) {
                String[] posicoes = linha.split(limitador);
                valor = posicoes[findAtribute(atributo).getPosicao()-redutorPosicao];
            }
