...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript('../../plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
//
    ...
    function MakeSelectDataTable(tableName) {
        ...
        $('.dataTables_filter').each(function () {
            $(this).find('label input[type=search]').attr('placeholder', 'Pesquisa r\u00e1pida');
            $(this).find('label input[type=search]').attr('class', 'form-control');
            $('#'+$(this).attr('id')).prepend('<div class="input-group" id="teste"><span class="input-group-addon"><i class="fa fa-search"></i></span></div>')
            $(this).find('label input[type=search]').prependTo($($(this).find('div.input-group')));   
            $('#'+$(this).attr('id')).addClass('dataTables_filter pull-left');
        });
    }
//
